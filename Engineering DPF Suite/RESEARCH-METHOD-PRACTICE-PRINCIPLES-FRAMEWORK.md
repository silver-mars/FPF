...
The relevant Method Engineering contributions supply reusable identity, trial, fit, worth, variant and introduction distinctions. [RMP.9's source account](#rmp911---sota-echoing) qualifies that use: the published ME.14 next-probe continuation and full record are not automatic RMP.9 obligations. The research decision chooses acquisition by its effect, attainability and whole burden.
...
The working question is how to improve research practice when criticism leaves a Method decision open, without spending effort on evidence that cannot change the answer. The best-known answer selected here combines situated comparison with a prior decision about whether further evidence is worth obtaining. [Method Engineering](METHOD-ENGINEERING-PRINCIPLES-FRAMEWORK.md), especially ME.14:4.1–4.2, supplies the serious existing alternative: compare feasible courses and their full burdens, then return a retained set and next probe when evidence cannot distinguish actions. RMP.9 **adopts** its identity, trial, fit, worth, variant and introduction contributions and **adapts** that continuation in §4.3: select acquisition by its decision effect, attainability and whole burden. It **rejects** an automatic next probe or full ME.14 record for every research-practice question. At the effort needed to diagnose §5.1's specified calculation error, retention plus verified repair already answers the use; a further efficacy study adds no discrimination. Where qualification is unresolved, this choice accepts continued uncertainty and restricts unsupported use. It does not claim to repair ME.14 for other users. Reopen this adaptation if a revised ME.14 supplies the same sufficient stopping boundary or an attainable comparison becomes worth its burden.
